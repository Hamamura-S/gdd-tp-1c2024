...
BEGIN
    ...
    SELECT
        ...
        (
            ...
            WHERE 
                YEAR(TICKET_FECHA_HORA) = (SELECT anio FROM BI_GESTIONANDING.BI_DIM_TIEMPO WHERE tiempo_id=dimt.tiempo_id ) AND 
                BI_GESTIONANDING.FX_OBTENER_CUATRIMESTRE(TICKET_FECHA_HORA) = (SELECT cuatrimestre FROM BI_GESTIONANDING.BI_DIM_TIEMPO WHERE tiempo_id=dimt.tiempo_id ) AND 
                MONTH(TICKET_FECHA_HORA) = (SELECT mes FROM BI_GESTIONANDING.BI_DIM_TIEMPO WHERE tiempo_id=dimt.tiempo_id )
                AND LOCALIDAD_ID = (SELECT localidad_cd FROM BI_GESTIONANDING.BI_DIM_UBICACION_SUCU WHERE ubicacion_sucu_id=dimus.ubicacion_sucu_id) 
                AND LOCALIDAD_PROVINCIA = (SELECT provincia_cd FROM BI_GESTIONANDING.BI_DIM_UBICACION_SUCU WHERE ubicacion_sucu_id=dimus.ubicacion_sucu_id)
                AND BI_GESTIONANDING.FX_OBTENER_TURNO(TICKET_FECHA_HORA) = (SELECT turno_id FROM BI_GESTIONANDING.BI_DIM_TURNO WHERE turno_id=dimtu.turno_id)
                AND CAJA_TIPO = c.CAJA_TIPO 
                AND BI_GESTIONANDING.FX_CALCULAR_RANGO_ETARIO(
                        DATEDIFF(YEAR, EMPLEADO_FECHA_NACIMIENTO, GETDATE())
                    ) = (SELECT rango_etario_id FROM BI_GESTIONANDING.BI_DIM_RANGO_ETARIO WHERE rango_etario_id=dimre.rango_etario_id)
        ),
        (
