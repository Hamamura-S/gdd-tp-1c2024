...
INSERT INTO BI_GESTIONANDING.DIM_UBICACION_CLIE (provincia, localidad)
    SELECT distinct PROV_NOMBRE, DIRE_LOCALIDAD
...
BEGIN
    INSERT INTO BI_GESTIONANDING.BI_FACT_ENVIO (
        ...
        -- dimensiones
            ...
            dimt.tiempo_id,
        -- calculables
            COUNT(ENVIO_NRO) AS cant_envios,
            SUM(
                CASE WHEN ENVIO_FECHA_HORA BETWEEN '08:00' AND '20:00' THEN 1 ELSE 0 END
        ...
        JOIN GESTIONANDING.SUCURSAL ON SUCURSAL_ID = TICKET_SUCURSAL
            JOIN BI_GESTIONANDING.DIM_SUCURSAL dims ON SUCURSAL_NOMBRE = nombre
            JOIN BI_GESTIONANDING.DIM_RANGO_ETARIO dimre ON
                BI_GESTIONANDING.FX_CALCULAR_RANGO_ETARIO(
                    DATEDIFF(YEAR, CLIENTE.CLIENTE_FECHA_NACIMIENTO, GETDATE())
                ) = dimre.descripcion
            JOIN BI_GESTIONANDING.DIM_UBICACION_CLIE dimuc ON
                LOCALIDAD_NOMBRE = dimuc.localidad AND PROVINCIA.PROV_NOMBRE = dimuc.provincia
            JOIN BI_GESTIONANDING.DIM_TIEMPO dimt ON
