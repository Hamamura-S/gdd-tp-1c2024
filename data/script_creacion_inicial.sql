...
BEGIN
    ...
    JOIN GESTIONANDING.TICKET t ON
        t.TICKET_NUMERO=m.TICKET_NUMERO AND
        t.TICKET_FECHA_HORA=m.TICKET_FECHA_HORA and
        t.TICKET_TIPO_COMPROBANTE=m.TICKET_TIPO_COMPROBANTE AND
        t.TICKET_SUBTOTAL_PRODUCTOS=m.TICKET_SUBTOTAL_PRODUCTOS AND
        t.TICKET_TOTAL_DESCUENTO_PROMOCIONES=m.TICKET_TOTAL_DESCUENTO_APLICADO and 
        t.TICKET_TOTAL_DESCUENTO_APLICADO_MP=m.TICKET_TOTAL_DESCUENTO_APLICADO_MP and
        t.TICKET_TOTAL_ENVIO=m.TICKET_TOTAL_ENVIO and
        t.TICKET_TOTAL_TICKET=m.TICKET_TOTAL_TICKET
...
BEGIN
    ...
    SELECT CLIENTE_ID, TARJETA_ID, PAGO_TARJETA_CUOTAS
    FROM
        (
            SELECT TICKET_NUMERO, TICKET_FECHA_HORA, TICKET_TIPO_COMPROBANTE,
                TARJETA_ID, PAGO_TARJETA_CUOTAS
            FROM gd_esquema.Maestra m1
            JOIN GESTIONANDING.TARJETA ta ON ta.TARJETA_NUMERO = m1.PAGO_TARJETA_NRO AND ta.TARJETA_VENCIMIENTO = m1.PAGO_TARJETA_FECHA_VENC
        ...
        (
            ...
            JOIN GESTIONANDING.CLIENTE c ON m2.CLIENTE_DNI = c.CLIENTE_DNI
            WHERE m2.CLIENTE_DNI IS NOT NULL
        ) AS CLIENTES
        ON TARJETAS.TICKET_NUMERO = CLIENTES.TICKET_NUMERO 
        AND TARJETAS.TICKET_FECHA_HORA = CLIENTES.TICKET_FECHA_HORA 
        AND TARJETAS.TICKET_TIPO_COMPROBANTE = CLIENTES.TICKET_TIPO_COMPROBANTE
END
GO
...
BEGIN
    ...
    FROM 
        ...
        --DATOS TICKETS
        JOIN GESTIONANDING.TICKET t 
            ON m1.TICKET_NUMERO = t.TICKET_NUMERO
            AND m1.TICKET_FECHA_HORA = t.TICKET_FECHA_HORA
            AND m1.TICKET_TIPO_COMPROBANTE = T.TICKET_TIPO_COMPROBANTE
...
BEGIN
    ...
    FROM gd_esquema.Maestra m
        ...
        JOIN GESTIONANDING.PAGO p
            ON p.PAGO_TICKET = t.TICKET_NUMERO
